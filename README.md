...
Options:
  ...
  -i, --include <INCLUDE>            Specifies the directory, including its subdirectories, where additional templates can be found. It's necessary when the `<TEMPLATE_FILE>` will import or include other templates. Note that any relative paths specified in the `import` or `include` statements within templates are resolved relative to the directories indicated by `--include`
